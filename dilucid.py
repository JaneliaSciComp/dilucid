            ...
            if do_it_for_reals :    
                if not has_been_run_on_one_file_in_this_folder :
                    ...
                    command_line = ( ( 'bsub -o "%s" -e "%s" -q gpu_any -n1 -gpu "num=1" singularity exec ' +
                                     ...
                                     % (stdout_file_path, stderr_file_path, source_file_path, network_file_path, lock_file_path, target_file_path) )
...
def process_single_network_folder(input_folder_path, output_folder_path, do_know_network_folder_path, network_folder_path_maybe, n_submitted) :
    # print to show progress
    print("%s:" % input_folder_path)

    # get a list of all files and dirs in the source, dest dirs
    try:
        source_entries = os.listdir(input_folder_path)
    ...
    names_of_source_folders = [source_entry 
                               for source_entry 
                               in source_entries 
                               if os.path.isdir(os.path.join(input_folder_path, source_entry))]
    names_of_source_files = [source_entry 
                             for source_entry 
                             in source_entries 
                             if os.path.isfile(os.path.join(input_folder_path, source_entry))]
    ...
    if is_empty(network_folder_path_maybe) :
        ...
        if "network" in names_of_source_folders :
            network_folder_path = os.path.join(input_folder_path, "network")
            network_folder_path_maybe = [ network_folder_path ]
            names_of_source_folders = [ item for item in names_of_source_folders if item!="network" ]
        else :
    ...
    # names_of_source_folders doesn't contain it (which is good)

    # Process the files in this folder
    n_submitted = process_files_in_one_folder(input_folder_path, 
                                              names_of_source_files, 
                                              output_folder_path, 
                                              network_folder_path, 
                                              n_submitted)
        
    # For each folder in names_of_source_folders, recurse
    for source_subfolder_name in names_of_folders_in_source_folder:
        n_submitted = process_single_network_folder(os.path.join(source_folder_path, source_subfolder_name),
...
def process_dilucid_root_folder(root_folder_path, root_output_folder_path):
    ...
    n_submitted = 0
    try :
        source_entries = os.listdir(root_folder_path)
    except FileNotFoundError :
    ...
    #     entry_path = os.path.join(root_folder_path, source_entry)
    #     if os.path.isdir(entry_path) :
    #         names_of_folders_in_source_folder.append(source_entry)            
    names_of_folders_in_root_folder = [source_entry 
                                       for source_entry 
                                       in source_entries 
                                       if os.path.isdir(os.path.join(root_folder_path, source_entry))]
